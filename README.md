...
We simulated the updated volatility strategy across several base exposures (gearing levels). The codebase utilizes two transaction-cost reduction thresholds: a **Rebalance Threshold** (0.05) and a **Signal Activation Threshold** (2.0).
...
> [!NOTE]
> **Backtest Extension (2008 Crash):** 
> With more time, we would extend the backtest back to 2008 to analyze how the strategy handles the Global Financial Crisis, which represents a historic, prolonged high-volatility regime and the ultimate stress test for tail-hedging portfolios.
